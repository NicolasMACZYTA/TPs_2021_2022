...
void initialiserGraphe(graphe_t *g, char *nomFichier){
	...
	if(fichier != NULL){
		...
		g->listeAdjacences=malloc(sizeof(liste_t)*g->nb_sommets);
		...
		for(i=0;i<g->nb_sommets;i++){
		    //liste_t l;
			//initialiser_liste(&l);
			initialiserListe(&g->listeAdjacences[i]);
		}
		...
		for(i=0;i<g->nb_sommets;i++){
			g->tSommet[i].idSommet=g->listeAdjacences[i].tete->idSommet;
...
void afficherGraphe(graphe_t *g){
	...
	if(g->nb_sommets!=-1){
		...
		for(i=0;i<g->nb_sommets;i++){
			printf(" %d --> ", g->listeAdjacences[i].tete->idSommet);
